...
import (
	sap_api_caller "sap-api-integrations-campaign-reads-rmq-kube/SAP_API_Caller"
	sap_api_input_reader "sap-api-integrations-campaign-reads-rmq-kube/SAP_API_Input_Reader"
...
func callProcess(caller *sap_api_caller.SAPAPICaller, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	campaignID, campaignName := extractData(msg.Data())
	accepter := getAccepter(msg.Data())
	caller.AsyncGetCampaign(campaignID, campaignName, accepter)
	return nil
}

func extractData(data map[string]interface{}) (campaignID, campaignName string) {
	sdc := sap_api_input_reader.ConvertToSDC(data)
	campaignID = sdc.CampaignCollection.CampaignID
	campaignName = sdc.CampaignCollection.CampaignName
	return
}
